...
@push('styles')
<style>
    .manual-requests .page-title { font-size: 1.5rem; font-weight: 700; text-align: center; margin-bottom: 0.35rem; color: var(--dtr-heading); }
    .manual-requests .page-sub { text-align: center; color: var(--dtr-muted); margin-bottom: 1.15rem; max-width: 32rem; margin-left: auto; margin-right: auto; line-height: 1.45; }
    ...
    .manual-requests .card .card-body { padding: 1rem 1.08rem 1.1rem; }
    @media (min-width: 992px) {
        .manual-requests .card .card-body { padding: 1.2rem 1.25rem 1.15rem; }
    }
    /* Filter + search — theme tokens, no glowing blue shell */
    ...
    .manual-requests .mrr-controls .filter-box .form-label {
        color: var(--dtr-muted);
        font-size: 0.65rem;
        font-weight: 700;
    ...
    .manual-requests .mrr-controls .form-select {
        ...
        font-size: 0.8625rem;
        font-weight: 500;
        padding: 0.34rem 1.9rem 0.34rem 0.65rem;
    ...
    .manual-requests .mrr-controls .form-select:focus {
        ...
        box-shadow: 0 0 0 2px color-mix(in srgb, var(--dtr-primary) 17%, transparent);
    }
    .manual-requests .mrr-controls .form-select option {
        background: var(--dtr-card-bg);
        color: var(--dtr-text);
    }
    .manual-requests .mrr-controls .search-wrap { margin: 0; min-width: 0; }
    .manual-requests .mrr-controls .search-form { position: relative; width: 100%; max-width: none; }
    .manual-requests .mrr-controls .search-input {
        width: 100%;
        min-height: 40px;
        padding: 0.42rem 2.35rem 0.42rem 2.5rem;
        font-size: 0.8625rem;
        border-radius: 10px;
        border: 1px solid var(--dtr-input-border);
    ...
    .manual-requests .mrr-controls .search-input:focus {
        outline: none;
        border-color: color-mix(in srgb, var(--dtr-primary) 48%, var(--dtr-input-border));
        box-shadow: 0 0 0 2px color-mix(in srgb, var(--dtr-primary) 17%, transparent);
    }
    .manual-requests .mrr-controls .search-icon { position: absolute; left: 0.75rem; top: 50%; transform: translateY(-50%); font-size: 0.92rem; color: var(--dtr-muted); pointer-events: none; }
    .manual-requests .mrr-controls .search-clear { position: absolute; right: 0.2rem; top: 50%; transform: translateY(-50%); border: none; background: transparent; color: var(--dtr-muted); }
    /* Table chrome */
    ...
    /* Checkbox column: consistent width + optical center vs row height */
    ...
    .manual-requests .mrr-checkbox-holder input[type="checkbox"]:checked {
        background-color: color-mix(in srgb, var(--dtr-primary, #059669) 85%, #059669);
        border-color: color-mix(in srgb, var(--dtr-primary, #059669) 78%, #059669);
    ...
    html[data-theme="dark"] .manual-requests .mrr-checkbox-holder input[type="checkbox"]:checked {
        border-color: color-mix(in srgb, var(--dtr-primary, #059669) 55%, rgba(226,232,240,0.35));
    }
    .manual-requests .mrr-checkbox-holder input[type="checkbox"]:focus-visible {
        outline: 2px solid color-mix(in srgb, var(--dtr-primary) 42%, transparent);
        outline-offset: 2px;
    }
    ...
    .manual-requests .mrr-table-frame .table.table-hover > tbody > tr:hover > td,
    .manual-requests .mrr-table-frame .table.table-hover > tbody > tr:hover > th {
        background: var(--dtr-hover-bg) !important;
    }
    .manual-requests .mrr-table-frame .table.table-hover > tbody > tr:hover .reason-cell-inner {
        box-shadow: inset 0 0 0 1px #86efac;
    }
    html[data-theme="dark"] .manual-requests .mrr-table-frame .table.table-hover > tbody > tr:hover .reason-cell-inner {
    ...
    .manual-requests .table.table-hover > tbody > tr:hover .times-list strong {
        color: var(--dtr-heading) !important;
    }
    ...
    /* Reason: shrink-wrap short text w/ even padding; long text wraps inside max width */
    ...
    .manual-requests .mrr-status--pending {
        color: #b45309;
        background: color-mix(in srgb, #fbbf24 16%, transparent);
        border-color: color-mix(in srgb, #f59e0b 42%, transparent);
    }
    .manual-requests .mrr-status--approved {
        color: #047857;
        background: color-mix(in srgb, #34d396 14%, transparent);
        border-color: color-mix(in srgb, #10b981 38%, transparent);
    }
    .manual-requests .mrr-status--rejected {
        color: #b91c1c;
        background: color-mix(in srgb, #f87171 12%, transparent);
        border-color: color-mix(in srgb, #ef4444 36%, transparent);
    }
    html[data-theme="dark"] .manual-requests .mrr-status--pending { color: #fde68a; }
    html[data-theme="dark"] .manual-requests .mrr-status--approved { color: #a7f3d0; }
    html[data-theme="dark"] .manual-requests .mrr-status--rejected { color: #fecaca; }
                                ...
                                <tr>
                                    ...
                                    <td class="mrr-status-col">
                                        <div class="mrr-status-stack">
                                            @if($row->status === \App\Models\ManualAttendanceRequest::STATUS_PENDING)
                                                <span class="mrr-status mrr-status--pending">Pending</span>
                                            @elseif($row->status === \App\Models\ManualAttendanceRequest::STATUS_APPROVED)
                                                <span class="mrr-status mrr-status--approved">Approved</span>
                                            @else
                                                <span class="mrr-status mrr-status--rejected">Rejected</span>
                                            @endif
                                            @if($row->reviewed_at)
                                        ...
                                        @if($row->status === \App\Models\ManualAttendanceRequest::STATUS_PENDING)
                                            <div class="manual-request-actions">
                                                <button type="button"
                                                        ...
                                                        data-action="{{ route('coordinator.manual.requests.review', $row) }}"
                                                        ...
                                                        data-confirm-message="Approve this request and post attendance for this date?"
                                                ...
                                                <button type="button"
                                                        ...
                                                        data-target-summary="{{ e(($row->student?->name ?? 'Unknown student').' — '.($row->attendance_date?->format('M j, Y') ?? '')) }}"
                                                        data-confirm-message="Reject this request? A note is recommended."
